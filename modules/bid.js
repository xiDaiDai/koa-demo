...
class BidModel {
    ...
    static async save(pageIndex){

        let url = 'http://snzb.minegoods.com/sdnycms/category/bulletinList.html?searchDate=1994-07-25&dates=300&word=&categoryId=2&tabName=%E6%8B%9B%E6%A0%87%E5%85%AC%E5%91%8A&industryName=&status=&page='+pageIndex
            ...
            if (!error && response.statusCode == 200) {
                ...
                $('.newslist > li').each(function (i, el) {
                    ...
                    $(this).find('.newsinfo li').each(function (i,el) {
                        if(i == 0){code = $(this).find('span').text();}
                        if(i == 1){method = $(this).find('span').text();}
                        if(i == 2){deadline = $(this).find('span').text();}
                    });
